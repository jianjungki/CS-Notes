...
聚簇索引不是一种单独的索引类型，而是一种数据存储方式，单InnoDB的聚簇索引实际上是同一个结构保存了B-Tree索引和数据行
...
- 测试适用于该表的 WHERE 条件部分。根据测试结果接受或拒绝该行。

##### 使用索引条件下推，扫描是这样进行的：

- 获取下一行的索引元组（但不是完整的表行）。
- 测试适用于该表的 WHERE 条件部分，并且可以仅使用索引列进行检查。如果不满足条件，则处理下一行的索引元组。
